...
var mainContensSelector = document.getElementsByClassName('section_mainOfMain');

function mainContentsBasic (){
    
    //현재역
    for(var i = 0; i<mainContensSelector.length; i++){
        
        var name_selector = '.this_station:eq('+i+')';
        $(name_selector).html(stationNames[i]);
        
    }
    
    //첫막차정보
    var y=0;
    for (var keyvalue in stationSetting){
        
        var first_for_Y = stationSetting[keyvalue].DepartY[0].substr(0,2) + " : " + stationSetting[keyvalue].DepartY[0].substr(2,2);
        
        document.getElementsByClassName('first_depart_Y')[y].innerHTML = "첫차 | " + first_for_Y;
        
        
        var last_for_Y = stationSetting[keyvalue].DepartY[1].substr(0,2) + " : " + stationSetting[keyvalue].DepartY[1].substr(2,2);
        
        document.getElementsByClassName('last_depart_Y')[y].innerHTML = "막차 | " + last_for_Y;
        
        
        var last_for_S = stationSetting[keyvalue].DepartY[2];
            if (last_for_S){
                last_for_S = stationSetting[keyvalue].DepartY[2].substr(0,2) + " : " + stationSetting[keyvalue].DepartY[2].substr(2,2);
                document.getElementsByClassName('last_depart_S')[y].innerHTML = "수성구청행 | " + last_for_S;
            }
            else {
                document.getElementsByClassName('last_depart_S')[y].innerHTML = "수성구청행 | " + "-----";
            }
        
        var first_for_M = stationSetting[keyvalue].DepartM[0].substr(0, 2) + " : " + stationSetting[keyvalue].DepartM[0].substr(2, 2);
        document.getElementsByClassName('first_depart_M')[y].innerHTML = "첫차 | " + first_for_M;
        
        var last_for_M = stationSetting[keyvalue].DepartM[1].substr(0, 2) + " : " + stationSetting[keyvalue].DepartM[1].substr(2, 2);
        document.getElementsByClassName('last_depart_M')[y].innerHTML = "막차 | " + last_for_M;
        
        var last_for_I = stationSetting[keyvalue].DepartM[2];
            if (last_for_I){
                last_for_I = stationSetting[keyvalue].DepartM[2].substr(0, 2) + " : " + stationSetting[keyvalue].DepartM[2].substr(2, 2)
                document.getElementsByClassName('last_depart_I')[y].innerHTML = "이곡행 | " + last_for_I;
            }
        else {
                document.getElementsByClassName('last_depart_I')[y].innerHTML = "이곡행 | " + "-----";
        }
        
        
        y++;
        
        if(y==mainContensSelector.length){
            y=y-1;
        }
    }
    ...
    // 역세권 풍경
    var z = 0;
    for(var keyvalueForPic in stationSetting){
        
        var pictureSelect = './img/station_back/' + keyvalueForPic + '1.jpg';
        var pictureSelect2 = './img/station_back/' + keyvalueForPic + '2.jpg';
        $('.hotPlace').eq(z).children('.left_place').children('p').eq(0).children('img').attr('src',pictureSelect);
        $('.hotPlace').eq(z).children('.right_place').children('p').eq(0).children('img').attr('src',pictureSelect2);
        
        var picNumber = ((Math.ceil(Math.random()*10))%2)+1;
        
        var url = 'url(./img/station_back/'+keyvalueForPic+ picNumber +'.jpg)'
        $('.main_content').children('.section').eq(z+1).css('background-image', url);
        
        
        
        z++;
        
        if(z==mainContensSelector.length){
            z=z-1;
        }
    }
    ...
    //역내부 정보
    var j= 0;
    for(var stationVal in stationSetting){
        if (stationSetting[stationVal].toilet === 1){
            $('.toilet').eq(j).removeClass('hide_facility');
        }
        
        if (stationSetting[stationVal].parking === 1){
            $('.parking').eq(j).removeClass('hide_facility');
        }
        
        if (stationSetting[stationVal].reverse === 1){
            $('.transfer').eq(j).removeClass('hide_facility');
            $('.fa-line-chart').eq(j).removeClass('hide_facility');
        }
        else if (stationSetting[stationVal].reverse === 2){
            $('.transfer').eq(j).removeClass('hide_facility');
            $('.fa-refresh').eq(j).removeClass('hide_facility');
        }
        
        if(stationSetting[stationVal].transfer === 1){
            $('.this_station').eq(j).css('border-bottom-color','red').css('border-right-color','red');
            
            $('.station_circle').eq(j).addClass('one');
        }
        if(stationSetting[stationVal].transfer === 3){
            $('.this_station').eq(j).css('border-bottom-color','gold').css('border-right-color','gold');
            
            $('.station_circle').eq(j).addClass('three');
...
function scroll_zero(){
    if (window.pageXOffset < (window.innerWidth/2)){
        $('.left_side').addClass('side_hide');
        $('.line_graph').addClass('graph_hide');
    }
    else {
        $('.left_side').removeClass('side_hide');
        $('.line_graph').removeClass('graph_hide');
    }
    
    var windowWidth = window.innerWidth;
    var sectionNumber = document.getElementsByClassName('section').length;
    
    if ((window.pageXOffset +200)> (windowWidth * (sectionNumber-1)))
        $('.right_side').addClass('side_hide');
    
    else {
        $('.right_side').removeClass('side_hide');
    }
...
$(window).scroll(scroll_zero);


// 버튼 변화
function side_btn_name(){
    var number = (Math.floor(window.pageXOffset/window.innerWidth))-2;
    
    if(number<0){
        $('.prev_station').children('p').html('지하철 내리기');    
    }
    else {
        $('.prev_station').children('p').html(stationNames[number]);
    }
    
    if(number+2>stationNames.length){
        $('.next_station').children('p').html('지하철 내리기');    
    }
    else if(window.pageXOffset==0 || window.pageXOffset < window.innerWidth/2){
        $('.next_station').children('p').html('지하철 타러가기');
    }    
    else {
        $('.next_station').children('p').html(stationNames[number+2]);
    }
    
    $('.station_circle').css('background-color','white');
    $('.station_circle').eq(number+1).css('background-color','red');
    
    
}

$(window).scroll(side_btn_name);

//바로가기
function search_station (){
    var searchInput = document.getElementById('searchInput').value;
    
    if (searchInput.indexOf("역")==(searchInput.length-1)){
    ...
    if (searchInput=="성서공단" || searchInput=="성서" || searchInput=="산업단지"){
        searchInput = "성서산업단지";
    }
    
    var stationNumber = stationNames.indexOf(searchInput);
    
    if (searchInput == ''){
        stationNumber = Math.floor(Math.random()*29);
    }

    if (stationNumber == -1){
        $('.alert_noName').removeClass('hide_alert');
        setTimeout(
        function(){
            $('.alert_noName').addClass('hide_alert');
        }, 2000);
        return false;
    }
    
    $('html, body').stop().animate({
        scrollLeft : window.innerWidth * (stationNumber+1) 
    })
}
